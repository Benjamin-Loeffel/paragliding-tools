...
from .horizon import horizon_and_svf, horizon_and_svf_parallel
...
from .report import (build_energy_3d, build_energy_3d_timeslider,
...
@contextmanager
def timed(label: str):
    """Phasen-Timing fürs Perf-Profiling (Ausgabe '[t] <label>: N.N s')."""
    t = time.perf_counter()
    try:
        yield
    finally:
        log.info("  [t] %s: %.1f s", label, time.perf_counter() - t)
...
def _horizon_cached(cfg: ThermalConfig, grid, dtm):
    ...
    if cfg.horizon_workers > 1:
        log.info("Horizont/SVF berechnen (einmalig, %d Prozesse) …", cfg.horizon_workers)
        horizon, az, svf = horizon_and_svf_parallel(
            dtm, grid.res, cfg.n_azimuth, cfg.horizon_max_steps, cfg.horizon_step_m,
            workers=cfg.horizon_workers)
    else:
        log.info("Horizont/SVF berechnen (einmalig) …")
        horizon, az, svf = horizon_and_svf(
            dtm, grid.res, cfg.n_azimuth, cfg.horizon_max_steps, cfg.horizon_step_m)
...
def run_phase_a(cfg: ThermalConfig) -> dict:
    ...
    with timed("terrain laden/resamplen"):
    ...
    # (energy_3d.html entfällt — der Zeit-Slider energy_3d_ideal_slider.html deckt den idealen Eintrag ab)
    ...
    # (die je-Cutoff-3D-HTMLs energy_3d_bis*h.html entfallen — der Zeit-Slider ersetzt sie)
    for h, f in cum:
    ...
    with timed("energy ideal-slider HTML"):
    ...
    try:
        ...
        with timed("Einstrahlung real + Wärme"):
        ...
        # (energy_3d_real.html entfällt — der real-Slider energy_3d_real_slider.html deckt das ab)
        ...
        with timed("energy real-slider HTML"):
    ...
    paths = {"png": str(png), "html": str(html),
             "energy_3d_ideal_slider": str(out / "energy_3d_ideal_slider.html"),
             "cum_png": str(cum_png), "out": str(out)}
